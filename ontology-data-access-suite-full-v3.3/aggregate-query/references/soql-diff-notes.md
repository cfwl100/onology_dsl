...
### B. `returns` 定长元组（AGGREGATE）

- 仅允许：`["GROUP_BY", "<alias>.<field>", "<resultAlias>"]`
- 仅允许：`["METRIC", "<function>", "<alias>.<field>|<alias>.*", "<resultAlias>"]`
- `METRIC` 至少 1 项。

### C. `mutation` 简化规则（AGGREGATE）
