\
#!/usr/bin/env python3
"""Structural validator for canonical OQL."""
...
def validate_conditions(node: Dict[str, Any], allowed_refs: Set[str]) -> None:
    ...
    if kind == "GROUP":
        ...
        for child in children:
            validate_conditions(child, allowed_refs)
        return
...
def validate_returns(items: List[Dict[str, Any]], allowed_refs: Set[str], operation: str) -> None:
    ...
    for item in items:
        ...
        if kind == "FIELDS":
            ...
            assert_true(all(isinstance(f, str) and f and f != "*" for f in fields), "FIELDS fields must be explicit names and cannot contain *")
        elif kind == "GROUP_BY":
...
def is_value_expression(value: Any) -> bool:
    return isinstance(value, dict) and isinstance(value.get("$fn"), str) and value["$fn"]
...
def validate_oql(data: Dict[str, Any], is_batch_item: bool = False, depth: int = 0) -> None:
    ...
    if "conditions" in data:
        validate_conditions(data["conditions"], object_aliases | relationship_aliases)

    if "returns" in data:
        validate_returns(data["returns"], object_aliases | relationship_aliases, op)
    if "orders" in data:
    ...
    if "sourceQuery" in data:
        ...
        for obj in data.get("objects", []):
            if "fromSource" in obj:
                assert_true(obj["fromSource"] in outputs, f"objects.fromSource must reference same-level sourceQuery.outputAs: {obj['fromSource']}")
...
def main() -> int:
    ...
    parser.add_argument("--input", required=True, help="OQL JSON path")
