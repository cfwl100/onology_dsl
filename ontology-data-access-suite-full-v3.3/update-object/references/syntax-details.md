...
- 操作边界：`UPDATE`（更新写入）。
- 关键边界：`objects` 必须恰好 1 个，`conditions + mutation.scope + mutation.set` 必填。

## 6. 最小合法样例（S-OQL 形态）

```json
{"version":"1.0","schemaRef":"demo","strict":true,"operation":"UPDATE","objects":[{"objectType":"Order","alias":"o"}],"conditions":["o.orderNo","EQ","SO-001"],"mutation":{"scope":"ONE","set":{"status":"PAID","updatedAt":{"$fn":"now"}}
```
