...
### B. `returns` 定长元组（UPDATE）

- 写操作边界下 `UPDATE` 禁止 `returns`。

### C. `mutation` 简化规则（UPDATE）

- 仅允许 `mutation.scope + mutation.set`。
- `mutation.scope ∈ {ONE, MANY}`，`mutation.set` 必须为非空对象。
- 禁止：`mutation.data`、`mutation.matchBy`、`mutation.items`。
