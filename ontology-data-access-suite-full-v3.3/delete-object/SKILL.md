...
仅生成 `DELETE` 的规范结果，并且优先使用本插件内的确定性脚本完成**同壳 S-OQL → canonical OQL** 的转换、组装与校验。
...
- 只负责 `DELETE`。
- `objects` 长度必须为 1。
- `conditions`、`mutation.scope` 必须存在。
- 不得出现 `mutation.set`、`mutation.data`。
