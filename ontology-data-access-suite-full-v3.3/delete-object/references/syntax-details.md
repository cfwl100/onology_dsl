...
- 操作边界：`DELETE`（删除写入）。
- 关键边界：`objects` 必须恰好 1 个，`conditions + mutation.scope` 必填，`scope` 仅 `ONE/MANY`。

## 6. 最小合法样例（S-OQL 形态）

```json
{"version":"1.0","schemaRef":"demo","strict":true,"operation":"DELETE","objects":[{"objectType":"Order","alias":"o"}],"conditions":["o.orderNo","EQ","SO-001"],"mutation":{"scope":"ONE"}
```
