...
### B. `returns` 定长元组（DELETE）

- 写操作边界下 `DELETE` 禁止 `returns`。

### C. `mutation` 简化规则（DELETE）

- 仅允许 `mutation.scope`，且 `scope ∈ {ONE, MANY}`。
- 禁止：`mutation.set`、`mutation.data`、`mutation.matchBy`、`mutation.items`。
