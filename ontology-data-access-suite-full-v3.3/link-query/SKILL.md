---
name: link-query
description: 处理通过单一关系类型获取关联对象的一跳读取请求。仅在语义明确是一跳关联获取、且不需要显式多跳路径时使用，并生成符合 S-OQL 生成层语法规范的 `LINK_QUERY` S-OQL。
---
# S-OQL 单跳关联生成插件
...
`LINK_QUERY` 仅允许 `FIELDS` 三元组（默认目标对象 alias）。 具体元组形态与字段位置约束详见 references。
