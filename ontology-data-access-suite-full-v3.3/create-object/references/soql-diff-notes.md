...
### B. `returns` 定长元组（CREATE）

- 写操作边界下 `CREATE` 禁止 `returns`。

### C. `mutation` 简化规则（CREATE）

- 仅允许 `mutation.data`（直接属性对象）。
- 禁止：`mutation.scope`、`mutation.set`、`mutation.matchBy`、`mutation.items`。
