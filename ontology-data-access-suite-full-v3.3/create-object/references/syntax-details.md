...
- 操作边界：`CREATE`（创建写入）。
- 关键边界：`objects` 必须恰好 1 个且 `mutation.data` 必填；禁止 `conditions/returns/orders/sourceQuery/relationships/linkQuery`。

## 6. 最小合法样例（S-OQL 形态）

```json
{"version":"1.0","schemaRef":"demo","strict":true,"operation":"CREATE","objects":[{"objectType":"Order","alias":"o"}],"mutation":{"data":{"orderNo":"SO-001","amount":100,"createdAt":{"$fn":"now"}}
```
