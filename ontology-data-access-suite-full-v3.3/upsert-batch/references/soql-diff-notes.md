...
### B. `returns` 定长元组（UPSERT/BATCH）

- 写操作边界下 `UPSERT` 与 `BATCH` 顶层均禁止 `returns`。

### C. `mutation` 简化规则（UPSERT/BATCH）

- `UPSERT`：仅允许 `mutation.matchBy + mutation.data`。
- `BATCH`：仅允许 `mutation.atomic + mutation.items`，`items` 必须非空，且子项不得再是 `BATCH`。
