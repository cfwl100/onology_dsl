...
- 操作边界：`UPSERT/BATCH`（存在则更新或批处理）。
- 关键边界：`UPSERT` 要求 `matchBy+data`；`BATCH` 要求 `atomic+非空items`，且 `items` 禁止 `BATCH`。

## 6. 最小合法样例（S-OQL 形态）

```json
{"version":"1.0","schemaRef":"demo","strict":true,"operation":"UPSERT","objects":[{"objectType":"Order","alias":"o"}],"mutation":{"matchBy":["orderNo"],"data":{"orderNo":"SO-001","status":"NEW","updatedAt":{"$fn":"now"}}
```
