---
name: oql_upsert
description: "Generate strict OQL DSL JSON for operation UPSERT. Use when user intent maps to UPSERT and needs machine-consumable ontology object operation payload with minimal ambiguity."
---
...
- 使用 `mutation.matchBy` 定义幂等匹配键。
- `mutation.set` 表示命中更新字段。
- `mutation.insert` 表示未命中创建字段。
