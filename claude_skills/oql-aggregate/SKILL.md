---
name: oql_aggregate
description: "Generate strict OQL DSL JSON for operation AGGREGATE. Use when user intent maps to AGGREGATE and needs machine-consumable ontology object operation payload with minimal ambiguity."
---
...
- 聚合逻辑必须放在 `aggregations`。
- `metrics[].as` 必填，避免结果字段歧义。
- 如有分组，使用 `aggregations.groupBy`。
...
```json
{
  ...
  "aggregations": {
    ...
    "metrics": [{"func": "COUNT", "param": "o", "property": "id", "as": "orderCount"}]
  },
  "returns": [{"type": "aggregation", "fields": ["customerId", "orderCount"]}]
}
```
