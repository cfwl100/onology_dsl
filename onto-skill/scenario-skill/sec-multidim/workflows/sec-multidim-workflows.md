...
自然语言委托格式见：`templates/oac-natural-language-request.md`。
...
第三步：请调用本体平台的数据访问能力，按 OAC 自然语言委托模板生成并执行 OQL。操作类型、对象、条件、返回字段由当前业务场景决定。
```
...
```text
请调用本体平台的数据访问能力，按 OAC 自然语言委托模板生成并执行 OQL：
- 操作类型：QUERY。
- 操作选择依据：用户已给完整组合维度，或多维模型支持归属过滤升维，因此不需要关系路径。
- 查询对象：按场景声明 grid g、cell c 等对象。
- 关系路径：无关系路径，不要生成 relationships。
- 过滤条件：组合维度场景使用 DIM_GRID、DIM_CELL；归属过滤场景仅使用 DIM_GRID。
- 返回字段：返回用户要求的维度和指标，例如 DIM_CELL、C_RSRP、C_PRB。
- 扩展说明：说明这是 SEC 多维模型查询，优先按 DAC 多维模型映射处理；如果支持 extensions，可表达 COMPOSITE_DIMENSION、dimensionLift、targetBackend=DAC。
- 结果处理：保留原始字段，结果为空即为空。
```

---

## 4. Workflow：两步关系主键解析 + 指标 QUERY
...
```text
第一步：自然语言委托平台执行 ASSOCIATION_QUERY，通过 grid g 的 GRID_ID=A 沿 locateIn 关系查询 cell c 的 CELL_ID。
...
第三步：自然语言委托平台执行 QUERY，查询 cell c，过滤条件为 c.CELL_ID 等于或属于第一步返回的 CELL_ID，返回 c.CELL_ID 和 c.C_PRB。
```
...
请调用本体平台的数据访问能力，按 OAC 自然语言委托模板生成并执行 OQL：
- 操作类型：AGGREGATE。
- 操作选择依据：用户要求按维度分组统计指标。
- 查询对象：按场景声明对象，例如 cell c。
- 过滤条件：包含用户指定维度条件和时间条件。
- 返回字段/聚合要求：明确 groupBy、metric、聚合后过滤和排序限制。
- 时间要求：明确本地时间/UTC/默认时间，并将时间范围写入过滤条件。
- 扩展说明：说明这是 SEC 多维聚合查询，优先按 DAC 多维模型映射处理。
```
