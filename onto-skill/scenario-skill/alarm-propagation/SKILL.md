...
本层只负责把用户问题整理成 `Ontology-based-planning-skill` 需要的 6 行顶层输入并委托给`Ontology-based-planning-skill`执行。
...
`## 可注入的 6 行片段生成规则` 是组装骨架。

外层 `SKILL.md` 在组装 `业务领域知识` 时，必须同时读取并合并 knowledge 文件中与本次问题相关的细粒度段落。
...
按照`## 5. 组装规则` 输出给 Planning 时，只输出 6 行，不追加 JSON、stepContracts、planningDelegationPackage 或其他中间结构。
