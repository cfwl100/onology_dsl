...
OAG 是本体子图检索能力，负责根据自然语言问题和 `ontologyId` 检索相关对象、属性、关系与函数候选，为后续 Planning、OAC、Function 调用提供可信来源。
...
脚本调用时将自然语言检索目标整理为 `--query`，并传入 `--ontologyId`：

```bash
python scripts/semantic_subgraph_search.py --query "<自然语言检索问题>" --ontologyId "<ontologyId>"
```
...
请执行本体子图检索。
本体ID：network@1.0
用户原始问题：查询网元A上的告警及其可能影响的业务。
业务场景：alarm-propagation
