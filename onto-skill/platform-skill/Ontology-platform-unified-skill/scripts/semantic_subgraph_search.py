...
def search_subgraph(ontologyId: str, query: str, similarityThreshold: float = 0.6, includeFunctions: int = 1,
                    ...
                    topK: int = 3, graphExpansionStrategy: str = "minimal", hopLimit: int = 3) -> dict:
...
def main() -> int:
    ...
    parser.add_argument("--include-actions", type=int, default=1, help="是否包含函数 (默认 1)")
    parser.add_argument("--seed-retrieval-mode", type=str, default="vector", help="是否包含函数 (默认 1)")
    parser.add_argument("--topK", type=int, default=3, help="是否包含函数 (默认 1)")
    parser.add_argument("--graph-expansion-strategy", type=str, default="minimal", help="是否包含函数 (默认 1)")
    parser.add_argument("--hopLimit", type=int, default=3, help="是否包含函数 (默认 1)")
    ...
    if args.query_json:
        try:
            ...
            include_actions = payload.get("includeActions") or args.include_actions
            seed_retrieval_mode = payload.get("seedRetrievalMode") or args.seed_retrieval_mode
            top_k = payload.get("topK") or args.topK
            graph_expansion_strategy = payload.get("graphExpansionStrategy") or args.graph_expansion_strategy
    ...
    result = search_subgraph(ontology_id, query, similarity_threshold, include_functions, include_actions,
                             seed_retrieval_mode, top_k, graph_expansion_strategy, hop_limit)
