...
description: 本体规划执行层。接收业务定制 Skill 已构造好的 7 段自然语言输入，直接解析流程级定制和步骤级定制，按默认或定制流程完成子图检索、基于子图任务规划、OAC 数据访问、Function 发现/执行和结果汇总。
allowed_tools:
metadata:
  pattern: pipeline
...
本层接收上层业务定制 Skill 已经构造好的 7 段自然语言输入，直接根据其中的流程级定制决定执行链路，并在每个步骤叠加步骤级定制进行执行。

本层不负责业务语义识别，不改写业务意图，不摘要业务定制文件内容，不根据业务定制文件路径读取文件，不构造新的 `planningContext`，也不设置单独的 S1 输入门禁步骤。

## 2. 7 段输入

业务定制 Skill 必须传入以下 7 段内容：
...
职责：根据 7 段输入中的本体ID、业务意图和 S2 业务增量规则获取本体子图。
...
规则：S4 只消费 S3 plannedTasks、S2 必要子图摘要和 7 段输入中的业务增量规则。S4 不重新解释用户原始问题，不重新推理 S3 已规划路径。OAC 最终业务输出只保留 `{objects, relationships}`；调试信息只在 debug 或失败时输出。默认不写临时 OQL 文件，优先使用内存参数或标准输入传递紧凑 JSON。
