...
description: 本体规划执行层。作为可被上层业务 Skill 定制的默认本体子图规划层，接收语义请求、业务定制输入或显式执行步骤，先整理规划上下文，再基于本体子图形成默认执行流程，并按步骤调用 Ontology-platform-unified-skill 执行子图检索、数据查询和函数调用。
allowed_tools:
metadata:
  pattern: pipeline
...
2. 整理 planning 可消费的执行上下文，包括原始问题、目标、实体、变量、约束、`ontologyId`、`schemaRef` 和步骤信息。
3. 基于默认本体子图流程生成、合并或检查执行步骤。
4. 按步骤调用 `Ontology-platform-unified-skill` 执行子图检索、数据访问和函数调用。
5. 理解本体子图结构，提取对象、属性、关系、函数候选和绑定依据。
6. 保留执行轨迹、绑定关系和平台返回结果。
7. 返回执行结果、缺失信息、空结果说明或失败原因。
...
|---|---|---|---|
| S1 | 输入整理与规划上下文构造 | `SUMMARY` | 保留原始问题，整理目标、实体、约束、时间范围、业务上下文、变量、`ontologyId`、`schemaRef`、后续是否需要本体访问或函数能力。 |
| S2 | 检索本体子图 | `OAG` | OAG 入参保持自然语言，优先使用用户原始问题或上层业务 Skill 注入的检索问题，并可附加必要业务上下文。 |
| S3 | 解析子图能力 | `SUMMARY` | 从子图中识别对象、属性、关系、函数、SOP 和候选路径。 |
| S4 | 生成数据访问步骤 | `OAC` | 如果目标需要读取对象实例或统计数据，基于子图结果生成数据访问步骤。 |
...
2. 整理 `goal`、`intent`、`entities`、`variables`、`constraints`、`ontologyId`、`schemaRef`、`steps`。
3. 判断后续是否需要本体子图、本体访问、函数发现或函数调用。
4. 识别缺失项，例如缺少 `ontologyId`、`schemaRef`、实体值、时间范围或返回要求。
5. 合并上层业务 Skill 注入的知识、变量和步骤改写。
...
如果没有完整 `steps`，先执行 S1 输入整理与规划上下文构造。若上层业务 Skill 提供 `knowledge`、`variables`、`constraints`、`stepOverrides`、`stepAppends`、`stepSkips`，合并到默认流程。
...
| `SUMMARY` | 输入整理、规划、汇总 | 本层内部处理，不调用原始 Tool |
...
- 用户原始问题。
- 上层业务 Skill 明确注入的自然语言检索问题。
- 用户原始问题 + 必要业务上下文，例如目标对象提示、业务场景、时间范围。

可选辅助格式：

- `先找相关子图，再按 SOP 规划任务。`
- `围绕【{用户原始问题或业务目标}】检索相关对象、属性、关系和函数候选。`
...
1. **输入分类**：判断是默认规划输入、显式步骤输入，还是业务定制输入。
2. **上下文整理**：没有完整 `steps` 时，整理原始问题、目标、变量、约束、`ontologyId`、`schemaRef` 和后续能力需求。
3. **定制合并**：合并业务知识、变量、步骤覆盖和默认流程。
4. **步骤确认**：没有步骤时生成默认步骤；有步骤时检查步骤契约。
5. **子图解析**：从 `nodes` 和 `edges` 建立对象、属性归属、关系方向和函数候选。
6. **步骤输入检查**：执行前确认当前步骤所需对象、关系、条件、参数是否充分。
7. **委托执行**：每步只调用 `Ontology-platform-unified-skill` 的一个能力。
8. **结果绑定**：只绑定前一步明确返回的字段，不创造新字段。
9. **失败和空结果处理**：失败按策略处理；空结果视为有效结果。
10. **结果汇总**：汇总默认步骤、业务覆盖、子图依据、执行状态、关键输入输出、未执行步骤和原因。
