...
你的职责是：接收上层业务定制 Skill 输入的完整业务定制内容，解析业务意图、流程级定制和步骤级定制，按照内置 S1-S7 默认执行步骤完成“本体子图检索 -> 基于子图任务规划 -> OAC 数据访问 -> Function 发现/执行 -> 汇总”的闭环。

本 Skill 运行时必须自包含，不依赖 `docs` 或 `references` 目录。`onto-skill/docs` 只作为产品文档，不作为运行时 Skill 上下文。本 Skill 必须在自身内容中说明如何解析输入、如何执行 S1-S7、每个步骤的标准输入、标准输出、执行规则和失败策略。

本 Skill 不负责业务语义识别；业务语义由上层业务定制 Skill 完成。上层业务定制 Skill 必须把业务知识、检索规则、规划规则、查询规则、Function 规则和失败策略完整写入输入内容，本层只解析和执行，不额外猜测、不额外补齐、不重新归纳业务文件。
...
- `已读取业务定制文件`：必须列出已读取的业务文件路径。路径用于说明业务知识来源，不要求本层再次读取该文件。
- `业务定制文件内容`：必须包含本次执行所需的业务规则原文或完整摘录，包括场景知识、子图检索规则、任务规划规则、查询内容、查询类型、返回要求、Function 规则和失败策略。本层只使用该内容，不再自行寻找业务文件。
- `流程级定制`：必须用自然语言明确执行哪些步骤、顺序是什么、跳过哪些步骤、是否追加步骤。推荐格式：`执行 S1 -> S2 -> S3 -> S4 -> S7；不执行 S5/S6 Function；每个方向独立执行；S4 空结果视为有效结果，不自动放宽条件重试。`
- `步骤级定制`：必须用自然语言逐步说明 S2/S3/S4/S5/S6/S7 的输入、输出、执行规则和失败策略。业务 Skill 不需要构造 JSON，但必须把每一步定制描述完整。
...
第一部分是**流程级定制**：决定本次 planning 执行哪些大步骤，包括子图检索、基于子图的任务规划、OAC 查询、Function 发现、Function 执行、汇总等步骤是全部执行、部分执行、跳过执行还是追加执行，以及步骤顺序是什么。

第二部分是**步骤级定制**：决定每个具体步骤的输入内容、输出内容、执行规则和失败策略。例如：

- S2 子图检索要检索哪些对象、字段、关系、函数，以及 OAG 返回结构中需要保留哪些字段。
- S3 基于子图规划时，如何结合本体子图和业务规划规则生成 OAC 查询任务或 Function 执行任务。
- S4 OAC 查询时，查询哪些对象、哪些关系路径、哪些字段、采用什么操作类型、如何过滤、如何处理空结果。
- S5/S6 Function 发现与执行时，如何选择函数、如何获取参数规格、如何组装参数、如何处理缺参。
- S7 汇总时，如何展示结果、如何解释空结果、如何输出缺失项或冲突项。
...
- flowCustomization：来自“流程级定制”
- stepCustomization：来自“步骤级定制”
...
- 长列表、字段列表、告警列表、对象列表只进入 `variables` 一次，后续步骤只引用变量。
- 解析流程级定制得到待执行步骤序列。
- 解析步骤级定制得到每一步的输入要求、输出要求、执行规则和失败策略。
...
业务定制内容：<S2 相关的业务检索规则摘录>
检索目标：<需要检索的对象类型、字段、关系、函数候选>
返回结构要求：<业务希望 OAG 返回或保留的节点字段、边字段、函数字段、缺失项字段>
```
...
- S2 的 `subgraphOutput`
- S1 的 `variables`
- S1 的 `customizationContent` 中与 S3 相关的规划规则
- 流程级定制中的步骤顺序、跳过和追加要求
- 步骤级定制中对 S3 的任务拆分和失败策略要求
...
- S2 的 `subgraphOutput`
- S3 的 `plannedTasks`
- S1 的 `customizationContent` 中与 S4 相关的查询规则
...
3. S2/S3/S4/S5/S6/S7 的标准输入输出由本 Skill 内置，运行时不重复展开大段模板。
4. 步骤级定制只表达业务差异、变量引用、返回要求和失败策略。
5. S4 不重新推理 S3 已规划出的关系路径和过滤条件。
6. OAC 默认不写临时 OQL 文件，优先使用内存参数或 stdin。
7. 只在 debug、失败定位或用户明确要求时输出完整 trace。
