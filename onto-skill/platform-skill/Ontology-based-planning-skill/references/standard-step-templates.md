...
本文档定义 Planning 层通用的 S2/S3/S4/S5/S6/S7 标准步骤模板。业务 Skill 不应在运行时重复展开这些模板全文，只需要在 `planningDelegationPackage.stepContracts` 中通过 `stepTemplateRef` 引用，并通过 `contractRef` 指向业务定制文件中的增量规则。

## 1. 使用原则

默认执行态使用 `traceMode=compact`。
...
只有在 `traceMode=debug`、校验失败、执行失败、缺少对象/字段/关系/函数/参数规格，或用户明确要求完整 stepTrace 时，才展开本文档中的模板全文。
...
- 子图为空：按业务 `failurePolicyRef` 处理，默认停止后续依赖步骤。
...
- 多条路径均可用：按业务 contractRef 的优先级选择，不在 S4 重推理。
...
- S4 只消费 variables、S2.subgraphOutput、S3.plannedTasks 和 contractRef 对应业务增量规则。
...
业务意图：<为什么需要函数能力>
函数来源：<OAG result.functions 或业务 contractRef 指定目标>
...
- 多个函数候选：按 contractRef 规则排序；仍无法确定时返回歧义项。
...
参数缺失策略：<来自 contractRef 或默认 missing>
期望输出：functionOutput
```
