...
本文件已包含 AGGREGATE 所需公共规则和最小示例，如有需要可读`schemas/oql-aggregate.schema.json`获取详细规则。
...
7. 生成 OQL JSON。
8. 调用 `scripts/execute_oac_operation.py --oac-json '<compact-json>' --message-type '<type>'`，脚本内部自动完成 OQL 校验。
