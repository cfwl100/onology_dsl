...
用 `scripts/execute_oac_operation.py --validate-only --input <file>` 校验生成的 OQL JSON；手册内已含最小示例。
...
## 输入与命令
- 复杂/长 OQL 用 `json.dumps(ensure_ascii=False)` 写 UTF-8 文件，`execute_oac_operation.py --validate-only --input <file>`；短 JSON 且 Shell 引号安全才用 `--oac-json`，解析报错改用 `--input` 复用同一文件。
