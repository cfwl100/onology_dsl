...
def call_function(physicalName: str, function_id: str, params: dict) -> dict:
    """调用函数并获取结果"""
    get_env(function_id, params, physicalName)
    ...
    url = f"http://api-gateway-mesh.{namespace}.svc.cluster.local:39071/{physicalName}/ontology/function/{function_id}/call"
    headers = {
        "accept": "*/*",
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if attempt < MAX_RETRIES - 1:
                print(f"call_function-[Retry] Attempt {attempt + 1} failed: {e}")
...
def get_env(function_id, params, physicalName):
    if os.environ.get("DEBUG_ONTO", "false").lower() == "true":
        debug_dir = os.path.join(os.getcwd(), "onto_tmp")
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        debug_file = os.path.join(debug_dir, f"function_debug_{timestamp}.json")
        try:
            ...
            warnings.warn(f"Failed to write debug file {debug_file}: {e}")
...
def judge_return_code(args, result):
    if result["success"]:
        try:
        ...
        output = json.dumps(
            {"message_type": "FUNCTION_RESULT", "resultMessage": result_message, "content": data["result"]},
            ensure_ascii=False)
    else:
        output = json.dumps(result, ensure_ascii=False)
    if args.output:
