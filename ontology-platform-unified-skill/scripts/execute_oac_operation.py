...
def apply_runtime_defaults(oql: dict[str, Any]) -> None:
    if not oql.get("version"):
        oql["version"] = os.environ.get("OQL_VERSION", "1.0")
    if not oql.get("schemaRef"):
        schema_ref = os.environ.get("ONTOLOGY_ID")
        if schema_ref:
...
def format_success(result: dict[str, Any], message_type: str) -> str:
    ...
    if isinstance(data, dict) and "data" in data:
        if data.get("code", "") != 20000:
            ...
            try:
                message_dict = json.loads(error_msg)
                content = message_dict.get("llmMessage", error_msg)
