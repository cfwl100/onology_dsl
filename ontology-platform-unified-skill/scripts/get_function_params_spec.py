...
def get_params_spec(ontology_id: str, function_id: str) -> dict:
    ...
    url = f"http://api-gateway-mesh.{namespace}.svc.cluster.local:39071/ontomanager/rest/api/v1/ontologies/{ontology_id}/function-types/{function_id}/query"
    session_id = os.environ.get("_AGENT_SESSION_ID", "")
    headers = build_headers(get_context_by_session(session_id), tenant_id)

    for attempt in range(MAX_RETRIES):
        try:
            simplified = get_function_header(headers, url)
            return {"success": True, "data": simplified}
        except Exception as e:
            if attempt < MAX_RETRIES - 1:
                print(f"get_params_spec-[Retry] Attempt {attempt + 1} failed: {e}")
...
def get_function_header(headers, url):
